...
*   **4.1 运维场景的典型特点 (Typical Characteristics)**
    *   **问题及诊断的复杂性 (Complexity)**
        *   **问题描述**：故障场景多样（实时/历史、在线/离线），工具依赖不一，问题描述存在差异。
        *   **真实案例**：系统crash分析场景
            ```
            场景：系统crash原因分析
            环境：远程调试机（ssh root@76.53.137.175）
            约束：仅分析日志，禁止在生产环境操作
            路径：core文件在/home/crash/ixgbe_core，源码在/home/crash/ixgbe_core/src
            工具：crash ./vmlinux vmcore
            输出：分析报告写入/opt/ixgbe_core_analysis.md
            ```
        *   **设计要点**：明确环境边界、工具路径、输出要求，避免模型误操作
