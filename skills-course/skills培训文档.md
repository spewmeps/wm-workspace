...
*   **4.1 运维场景的典型特点 (Typical Characteristics)**
    *   **问题及诊断的复杂性 (Complexity)**
        *   **问题描述**：故障场景多样（实时故障/历史故障、在线分析/离线分析），依赖工具不一（通用/独有），且问题描述存在差异。
        参考如下真实场景示例，主要包含以下关键信息：
        
        1. **故障场景**：系统发生crash，需要分析原因
        2. **分析环境**：使用远程调试机器，不在真实环境执行操作，仅分析日志，避免模型做多余的动作。
        3. **文件目录**：明确指定日志路径，避免模型反复查询文件系统

        > **示例场景：** 我有个系统发生了crash，请帮我分析下原因。我将vmcore相关文件和命令部署到我的远程调试机，我的远程调试机登录方式：`ssh root@76.53.137.175` **重要提醒**：你所有的操作都要在此远程调试机执行，以下描述的所有目录也是远程调试机的目录。进入到远程调试机以后，core文件的目录如下：`cd /home/crash/ixgbe_core`（命令参考：`crash ./vmlinux vmcore`），同时 `/home/crash/ixgbe_core/src` 里面包含源码文件，分析下具体的源码问题。请将最后的结果输出到报告：`/opt/ixgbe_core_analysis.md`
